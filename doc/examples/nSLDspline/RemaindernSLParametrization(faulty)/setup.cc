...
#define PROTEXCH 3.02e-6
#define PROTNOTEXCH 1.82e-6
#define PROT2EXCH 7.35e-6
#define PROT2NOTEXCH 6.16e-6
...
void fnSetContrastProtein(double sigma, double global_rough, double rho_substrate, double nf_tether, double mult_tether, double l_tether, double l_lipid1, double l_lipid2, double vf_bilayer, double nf_lipid_2, double nf_lipid_3, double nf_chol, double radius_defect, double nf_protein, double protexchratio, fitinfo *fit, int contraststart, int contrastend){
    
    double v1,v2,dMaxArea;
    int i,j;
    double protvolume, protminnsl, protmaxnsl, maxnsldifference;
    double protatedproteinnsld, deuteratedproteinnsld;
    double lastprotvolume, lastnsl, lastnsldifference, currentprotvolumedifference, currentnsldifference;
    ...
    for (i=contraststart; i<contrastend+1; i++) {
        ...
        protatedproteinnsld=(PROTEXCH*protexchratio+PROTNOTEXCH*(1-protexchratio));
        deuteratedproteinnsld=(PROT2EXCH*protexchratio+PROT2NOTEXCH*(1-protexchratio));
        protvolume=protein.fnGetVolume(protein.dp[0],protein.dp[CONTROLPOINTS-1]);
        protminnsl=protvolume*protatedproteinnsld;
        protmaxnsl=protvolume*deuteratedproteinnsld;
        maxnsldifference=protmaxnsl-protminnsl;
        
        for (j=1; j<CONTROLPOINTS; j++) {
            lastprotvolume=protein.fnGetVolume(protein.dp[0],protein.dp[j-1]);
            lastnsl=protein.fnGetnSL(protein.dp[0],protein.dp[j-1]);
            lastnsldifference=lastnsl-lastprotvolume*protatedproteinnsld;
            currentprotvolumedifference=protein.fnGetVolume(protein.dp[j-1],protein.dp[j]);
            currentnsldifference=(maxnsldifference-lastnsldifference)*frac2_on[j];
            printf("j %i protvolume %g lastprotvolume %g lastnsl %g lastnsldifference %g currentprotvoldiff %g currentnsldiff %g maxnsldifference %g protmaxnsl %g protminnsl %g protatedproteinnsld %g deuteratedproteinnsld %g \n",j, protvolume, lastprotvolume, lastnsl, lastnsldifference, currentprotvolumedifference, currentnsldifference, maxnsldifference, protmaxnsl, protminnsl, protatedproteinnsld, deuteratedproteinnsld);
            if (currentprotvolumedifference!=0) {
                sld_on[j]=protatedproteinnsld+currentnsldifference/currentprotvolumedifference;
            }
            else {
                sld_on[j]=protatedproteinnsld;
            }
            protein.sld[j]=sld_on[j];
        }
...
void constr_models(fitinfo *fit)
{
    ...
    */
    ...
    fnSetContrastProtein(sigma, global_rough, fit[0].m.rho[3], nf_tether, mult_tether, l_tether, l_lipid1+dl_lipid_gcrinse1, l_lipid2+dl_lipid_gcrinse1, vf_bilayer_gcrinse1, nf_lipid_2, 0.0, 0.0, radius_defects, 1, protexchratio, fit, 2, 2);
    
    //------------- second incr----------------------------------------------------------------
    
    fnSetContrastProtein(sigma, global_rough, fit[0].m.rho[3], nf_tether, mult_tether, l_tether, l_lipid1+dl_lipid_gcrinse1, l_lipid2+dl_lipid_gcrinse1, vf_bilayer_gcrinse1, nf_lipid_2, 0.0, 0.0, radius_defects, fraction_inc2, protexchratio, fit, 3, 3);
    
    
    //--------------third inc-----------------------------------------------------------------------------
    fnSetContrastProtein(sigma, global_rough, fit[0].m.rho[3], nf_tether, mult_tether, l_tether, l_lipid1+dl_lipid_gcrinse1, l_lipid2+dl_lipid_gcrinse1, vf_bilayer_gcrinse1, nf_lipid_2, 0.0, 0.0, radius_defects, fraction_inc3, protexchratio, fit, 4, 4);
